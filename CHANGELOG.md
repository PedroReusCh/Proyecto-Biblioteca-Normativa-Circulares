...
* **Arquitectura Desacoplada de Tablas e Imágenes con Manifiestos Ligeros e IDs Canónicos**:
  ...
  * **Nuevo ETL Modular `ModificacionesPosterioresExtractor` (`scripts/extractors/modificaciones_posteriores.py`)**: Extractor dinámico independiente que captura notas marginales de vigencia y timbres de modificación posterior como **texto libre** flexible (ej. `"Circular Modificada por Circular Ord. N°214, de fecha 02 de mayo de 2024, DDU 498"`), integrándolo en el CSV maestro y esquemas semánticos.
  ...
  * **Extracción Estructurada de Firma (Nombre y Cargo Separados)**: Se rediseñó `FirmaExtractor` para desglosar y extraer independientemente `nombre_firmante`, `cargo_firmante` y el consolidado `firmante`, buscando de forma prioritaria el nombre o identificador de rúbrica en las líneas situadas arriba del cargo (`nombre_firmante = "JPB"` en DDU 456, `"JUAN DIEGO IZQUIERDO HEVIA"` en DDU 546), limpiando distorsiones OCR y preservando mayúsculas/minúsculas según la tipografía original.
